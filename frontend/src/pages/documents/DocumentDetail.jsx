...
import { Container, Row, Col, Card, Button, Badge, Alert, ListGroup, Tab, Nav, Modal, Form } from 'react-bootstrap';
...
const DocumentDetail = () => {
  ...
  const fetchDocumentDetails = async () => {
    try {
      ...
      const docResponse = await documentService.getDocumentById(currentUser.token, id);
      setDocument(docResponse.data);
      
      // Fetch document versions
      const versionsResponse = await documentService.getDocumentVersions(currentUser.token, id);
  ...
  const fetchSignatureStatus = async (signatureId) => {
    try {
      ...
      const response = await documentService.checkSignatureStatus(currentUser.token, signatureId);
  ...
  const handleDownload = async () => {
    try {
      const response = await documentService.downloadDocument(currentUser.token, id);
    ...
    <Container fluid className="py-4">
      <Row className="mb-4">
        ...
        <Col xs="auto" className="d-flex align-items-center">
          ...
          {document.status !== 'Signed' && document.status !== 'Awaiting Signature' && (
            <Button 
              variant="primary"
          ...
          <Card className="shadow-sm mb-4">
            ...
            <Card.Body>
              <Tab.Content>
                <Tab.Pane eventKey="details" active={activeTab === 'details'}>
                  <Row>
                    <Col md={6} className="mb-4">
                      ...
                      <Badge bg={getStatusBadge(document.status)} className="fs-6">
                        {document.status}
                      </Badge>
                    </Col>
                    ...
                    <Col md={6} className="mb-4">
                      ...
                      <p className="mb-0">{document.fileType?.toUpperCase() || 'Unknown'}</p>
                    </Col>
                    
                    <Col md={6} className="mb-4">
                      <h5 className="text-muted mb-2">Created By</h5>
                      <p className="mb-0">{document.createdBy?.name || 'Unknown'}</p>
                    </Col>
                    ...
                    {document.expiryDate && (
                      <Col md={6} className="mb-4">
                        ...
                        <p className="mb-0">{new Date(document.expiryDate).toLocaleDateString()}</p>
                      </Col>
                    )}
                    
                    <Col md={12} className="mb-4">
                      <h5 className="text-muted mb-2">Description</h5>
                      <p className="mb-0">{document.description || 'No description provided'}</p>
                    </Col>
                  </Row>
                </Tab.Pane>
                      ...
                      {versions.map((version, index) => (
                        <ListGroup.Item key={index} className="py-3">
                          <div className="d-flex justify-content-between align-items-center">
                            <div>
                              <h6 className="mb-1">Version {version.versionNumber}</h6>
                              <p className="text-muted mb-0 small">
                                Updated on {new Date(version.createdAt).toLocaleString()}
                            ...
                            <Button 
                              ...
                              onClick={() => {
                                ...
                                documentService.downloadDocumentVersion(currentUser.token, id, version.versionNumber)
                                  .then(response => {
                                    const url = window.URL.createObjectURL(new Blob([response.data]));
                                    const link = document.createElement('a');
                                    link.href = url;
                                    link.setAttribute('download', `${document.name}_v${version.versionNumber}`);
                                    document.body.appendChild(link);
                                    link.click();
                                    link.remove();
                                  })
                ...
                <Tab.Pane eventKey="signatures" active={activeTab === 'signatures'}>
                  {signatureLoading ? (
                    <div className="text-center py-4">
                    ...
                    <div className="text-center py-4">
                      <p className="text-muted mb-3">No signature requests found for this document</p>
                      <Button 
                        variant="primary" 
                        size="sm"
                        onClick={() => setShowSignatureModal(true)}
                    ...
                    <div>
                      <div className="mb-4">
                        ...
                        <Badge 
                          bg={signatureStatus.status === 'completed' ? 'success' : 'info'}
                          className="fs-6"
                        >
                          {signatureStatus.status === 'completed' ? 'Completed' : 'In Progress'}
                        </Badge>
                        {signatureStatus.expiresAt && (
                          <p className="text-muted mt-2 mb-0">
                            Expires on {new Date(signatureStatus.expiresAt).toLocaleDateString()}
                          </p>
                        )}
                      </div>
                          ...
                          <ListGroup.Item key={index} className="py-3">
                            <div className="d-flex justify-content-between align-items-center">
                              <div>
                                ...
                                <p className="text-muted mb-0 small">{sig.email}</p>
                              </div>
                              <Badge 
                                bg={sig.status === 'signed' ? 'success' : sig.status === 'viewed' ? 'warning' : 'secondary'}
                              >
                                {sig.status === 'signed' ? 'Signed' : 
                                 sig.status === 'viewed' ? 'Viewed' : 'Pending'}
                              </Badge>
                            </div>
                            {sig.signedAt && (
                              <p className="mt-2 mb-0 small">
                                <strong>Signed:</strong> {new Date(sig.signedAt).toLocaleString()}
                              </p>
                            )}
                          </ListGroup.Item>
                        ))}
                      ...
                      {signatureStatus.status !== 'completed' && (
                        <div className="d-grid">
                          <Button 
                            variant="outline-primary" 
                            size="sm"
                            onClick={() => fetchSignatureStatus(document.signatureId)}
                          >
                            Refresh Status
                          </Button>
                        </div>
          ...
          <Card className="shadow-sm mb-4">
            <Card.Body>
                ...
                <ListGroup.Item className="px-0 py-2 border-0">
                  <Button 
                    ...
                    onClick={() => {
                      ...
                      documentService.shareDocument(currentUser.token, id, { expiryDays: 7 })
                        .then(response => {
              ...
              <div className="mb-2 d-flex justify-content-between">
                ...
                <span>{document.fileSize ? `${(document.fileSize / 1024).toFixed(2)} KB` : 'Unknown'}</span>
              </div>
              <div className="mb-2 d-flex justify-content-between">
            ...
            <Card className="shadow-sm mb-4 border-info">
              <Card.Body>
                ...
                <p className="text-muted">
                  A signature request has been sent to the designated signatories. You'll be notified once the document is signed.
                </p>
                <Button 
      ...
      <Modal show={showSignatureModal} onHide={() => setShowSignatureModal(false)} size="lg">
        ...
        <Modal.Body>
          <p className="text-muted mb-4">
            Add signatories who need to sign this document. Each signatory will receive an email with instructions to complete the signature process.
          </p>
          ...
          <div className="d-flex align-items-center mb-3">
            ...
            <div>
              ...
              <p className="text-muted mb-0 small">
                All signatures are legally binding and comply with eSign regulations
              </p>
            </div>
          </div>
        </Modal.Body>
